...
add_const = False
model_postfix_base = "Stk_diam_ablation_study"
log_features_spl_base = (
    "relative_roughness",
    "sedimentation_Stk",
    "particle_droplet_diameter_ratio",
)
minmax_features_spl_base = ("inclination", "volume_fraction")
passthrough_features_spl_base = ("wettability",)

all_features = list(
    log_features_spl_base
    + minmax_features_spl_base
    + passthrough_features_spl_base
    + ("K",)
)
...
def get_train_test(
    ...
    * target - target for ML
    ...
    df_indexes = pd.read_excel(index_path, index_col=[0])
...
def load_df(
    *,
    ...
    df.drop(columns=target_set - {target}, inplace=True)
    if verbose:
...
def process_combination(current_features, number_of_features, estimator_class):
    """Process a single feature combination with a given estimator."""
    log_features_spl, minmax_features_spl = [], []
    passthrough_features_spl = []

    for feature in current_features:
        if feature in log_features_spl_base:
            log_features_spl.append(feature)
        if feature in minmax_features_spl_base:
            minmax_features_spl.append(feature)
        if feature in passthrough_features_spl_base:
            passthrough_features_spl.append(feature)

    current_drop = set(all_features) - set(
        log_features_spl + minmax_features_spl + passthrough_features_spl
    )
    if "K" in current_features:
        current_drop = current_drop - set("K")
    features_to_drop = features_to_drop_base + tuple(current_drop)

    # Initialize the estimator
    if estimator_class == "CatBoost":
    ...
    ml_pipe = MLPipeline(
        ...
        estimator=estimator,
        features_to_drop=features_to_drop,
        minmax_features=minmax_features_spl,
        passthrough_features=passthrough_features_spl,
        log_features=log_features_spl,
        add_const=add_const,
        model_postfix=f"ablation",
    )

    model_name = ml_pipe.model_name.split("_")[0]
    ml_pipe.run(save_model_and_metrics=False, verbose=False)

    return {
        "n_features": number_of_features,
        "model": model_name,
        "log": log_features_spl,
        "minmax": minmax_features_spl,
        "passthrough": passthrough_features_spl,
        "drop": features_to_drop,
        "holdout_test_f1_macro": ml_pipe.df_results["holdout_test_f1_macro"].values[0],
        "cv_test_f1_macro": ml_pipe.df_results["cv_test_f1_macro"].values[0],
        "cv_test_f1_macro_mean": ml_pipe.df_results["cv_test_f1_macro_mean"].values[0],
        "cv_test_f1_macro_median": ml_pipe.df_results["cv_test_f1_macro_median"].values[
            0
        ],
    }
...
def main():
    ...
    for number_of_features in range(1, len(all_features) + 1):
        sum_combs += math.comb(len(all_features), number_of_features)
    sum_combs *= 2  # For two estimators

    # Prepare all tasks
    tasks = []
    for number_of_features in range(1, len(all_features) + 1):
        for current_features in combinations(all_features, number_of_features):
            tasks.append((current_features, number_of_features, "CatBoost"))
            tasks.append((current_features, number_of_features, "StatsModels"))

    # Process with multiprocessing
    results = []
    with Pool(processes=cpu_count() - 1) as pool:  # Leave one core free
        with tqdm(total=sum_combs) as pbar:
            for result in pool.starmap(process_combination, tasks):
                results.append(result)
                pbar.update(1)
                pbar.set_description(
                    f"Processed {len(results)}/{sum_combs} combinations"
                )

    # Create DataFrame from results
    df_splashing_feature_selection = pd.DataFrame(results)
    df_splashing_feature_selection.to_excel("temp.xlsx")
